...
from todone.backend.db import Folder
from todone.backend.commands.folder import folder_command
...
class TestFolderCommand(DB_Backend):
    ...
    def test_new_outputs_action_taken(self):
        ...
        with redirect_stdout(f):
            folder_command({'subcommand': 'new', 'folders': ['test']})
        s = f.getvalue()
    ...
    def test_raises_when_rename_with_one_folder(self):
        with self.assertRaises(exceptions.ArgumentError):
            folder_command({'subcommand': 'rename', 'folders': ['today']})
    ...
    def test_delete_with_no_folders_raises(self):
        with self.assertRaises(exceptions.ArgumentError):
            folder_command({'subcommand': 'delete'})
    ...
    def test_delete_raises_when_folder_does_not_match(self):
        with self.assertRaises(exceptions.ArgumentError):
            folder_command({'subcommand': 'delete', 'folders': ['foo']})
    ...
    def test_delete_outputs_action_taken(self):
        f = io.StringIO()
        with redirect_stdout(f):
            folder_command({'subcommand': 'delete', 'folders': ['today']})
        s = f.getvalue()
    ...
    def test_list_displays_all_folders(self):
        f = io.StringIO()
        with redirect_stdout(f):
            folder_command({'subcommand': 'list'})
        s = f.getvalue()
    ...
    def test_list_raises_with_extra_arguments(self):
        with self.assertRaises(exceptions.ArgumentError):
            folder_command({'subcommand': 'list', 'folders': ['foo']})
