...
def list_items(args):
    ...
    today:   restricts to today todos
    next:    restricts to next todos
    inbox:   restricts to inbox todos
    remind:  restricts to reminder todos
    remind+N{d|w|m|y}:
                restricts to reminders with dates within the
                specified interval (N an integer)
    due:     restricts to todos with a due date
    due+N{d|w|m|y}: 
                restricts to todos due within specified interval
    all:     every active todo matches the search criteria (done and
